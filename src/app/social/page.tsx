...
export default function SocialIntelligencePage() {
  ...
  return (
    <div className="flex-1 p-6 md:p-10 max-w-7xl mx-auto w-full space-y-8">
      ...
      <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-6 border-b border-[#1e222a] pb-6">
        <div>
        ...
        <div className="flex items-center gap-3 flex-wrap">
          <button
            ...
            className="px-3.5 py-2 rounded-xl bg-[#161a22] hover:bg-[#1e2430] border border-[#262c37] text-neutral-200 text-xs font-medium flex items-center gap-2 transition-all shadow-sm"
          >
      ...
      {/* Navigation Tabs */}
      <div className="flex border-b border-[#1e222a] gap-2 overflow-x-auto pb-px">
        {[
          ...
          return (
            ...
            >
              ...
              <span>{tab.label}</span>
              {tab.badge !== undefined && (
                <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-[#1e2430] text-neutral-300 font-mono">
                  {tab.badge}
                </span>
              )}
            </button>
      ...
      {activeTab === "bank" && (
        <div className="space-y-6">
          {/* Filter Bar */}
          <div className="p-4 rounded-2xl bg-[#12151b] border border-[#202530] flex flex-wrap items-center justify-between gap-4">
            <div className="flex flex-wrap items-center gap-2">
              <span className="text-[11px] font-semibold text-neutral-400 uppercase tracking-wider flex items-center gap-1.5 mr-2">
              ...
              {[
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs transition-all ${
                    statusFilter === s.id
                      ? "bg-amber-600/20 border border-amber-500/50 text-amber-300 font-semibold"
                      : "bg-[#171b23] border border-[#262c37] text-neutral-400 hover:text-neutral-200"
                  }`}
                >
                  {s.label}
                </button>
            ...
            <form onSubmit={handleSearch} className="flex items-center gap-2 w-full md:w-auto">
              <select
                ...
                className="bg-[#171b23] border border-[#262c37] rounded-lg px-3 py-1.5 text-xs text-neutral-300 focus:outline-none focus:border-amber-500"
              >
              ...
              <div className="relative flex-1 md:w-64">
                ...
                <input
                  ...
                  className="w-full bg-[#171b23] border border-[#262c37] rounded-lg pl-8 pr-3 py-1.5 text-xs text-neutral-200 placeholder-neutral-500 focus:outline-none focus:border-amber-500"
                />
              </div>

              <button
                type="submit"
                className="px-3 py-1.5 rounded-lg bg-[#1e2430] text-neutral-300 hover:text-white text-xs border border-[#2b3545]"
              >
                Filter
              </button>
            </form>
          ...
          {isLoading ? (
            ...
            <div className="p-12 text-center rounded-2xl bg-[#13161c] border border-dashed border-[#262c37] space-y-3">
              <Layers className="w-8 h-8 text-neutral-500 mx-auto" />
              <div className="text-sm font-semibold text-neutral-200">No captions found in this view</div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
              {bankItems.map((item) => (
                <div
                  key={item.id}
                  className="p-5 rounded-2xl bg-[#13161d] border border-[#222834] hover:border-[#2f384a] transition-all flex flex-col justify-between space-y-4 shadow-sm"
                >
                  <div className="space-y-3">
                    {/* Badges & Meta */}
                    <div className="flex items-center justify-between gap-2 flex-wrap text-[11px]">
                      <div className="flex items-center gap-2">
                        <span className="px-2 py-0.5 rounded-md bg-amber-500/10 border border-amber-500/30 text-amber-400 font-medium">
                          {item.contentPillar}
                        </span>
                        <span className="px-2 py-0.5 rounded-md bg-[#1c222c] text-neutral-400 border border-[#2b3545]">
                          {item.contentType}
                        </span>
                      </div>
                    ...
                    {/* Title */}
                    <h3 className="text-sm font-bold text-neutral-100 leading-snug">{item.title}</h3>

                    {/* Caption Preview */}
                    <p className="text-xs text-neutral-300 leading-relaxed line-clamp-5 whitespace-pre-line font-normal">
                      {item.caption}
                    </p>
                    ...
                    {item.hashtags && item.hashtags.length > 0 && (
                      <div className="flex flex-wrap gap-1.5 pt-1">
                        {item.hashtags.map((h: string, idx: number) => (
                          <span key={idx} className="text-[11px] text-amber-400/80 font-mono">
                            {h}
                          </span>
                        ))}
                      </div>
                  ...
                  {/* Actions Bar */}
                  <div className="pt-3 border-t border-[#1e2430] space-y-3">
                    {/* Distinct Copy Buttons */}
                    <div className="flex items-center gap-2 flex-wrap">
                      <button
                        onClick={() => handleCopy(item, "CAPTION")}
                        className="px-2.5 py-1.5 rounded-lg bg-[#181c24] hover:bg-[#202632] border border-[#2a3242] text-neutral-300 text-[11px] flex items-center gap-1.5 transition-all"
                      ...
                      <div className="flex items-center gap-2">
                        {item.status !== "approved" && (
                          <button
                            ...
                            className="px-2.5 py-1 rounded bg-emerald-600/20 text-emerald-400 hover:bg-emerald-600/30 border border-emerald-500/30 font-medium"
                          >
                            Approve
                          </button>
                        )}
                        ...
                        <button
                          ...
                          className="text-neutral-400 hover:text-sky-400 transition-colors p-1"
      ...
      {activeTab === "gaps" && (
        <div className="space-y-6">
          <div className="p-6 rounded-2xl bg-[#12151b] border border-[#202530] space-y-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-2">
              <div className="p-4 rounded-xl bg-amber-950/20 border border-amber-500/30 space-y-2">
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {analytics?.contentGaps?.underrepresentedPillars?.map((p: string) => (
                    <span key={p} className="px-2.5 py-1 rounded bg-[#161a22] border border-[#2a3242] text-neutral-200 text-xs font-medium">
                      {p}
                    </span>
                  ))}
                </div>
              ...
              <div className="p-4 rounded-xl bg-[#161a22] border border-[#242b38] space-y-2">
                ...
                <div className="flex flex-wrap gap-2 pt-1">
                  {analytics?.contentGaps?.overusedPillars?.map((p: string) => (
                    <span key={p} className="px-2.5 py-1 rounded bg-[#12151b] border border-[#202530] text-neutral-400 text-xs">
                      {p}
                    </span>
                  ))}
                </div>
          ...
          <div className="space-y-3">
            <h3 className="text-sm font-semibold text-neutral-200">The 20 Brand Pillars Distribution</h3>
            <div className="grid grid-cols-2 sm:grid-cols-4 lg:grid-cols-5 gap-3">
              {ALL_CONTENT_PILLARS.map((p) => {
                const count = analytics?.contentGaps?.pillarCounts?.[p] || 0;
                return (
                  <div key={p} className="p-3.5 rounded-xl bg-[#13161c] border border-[#222834] space-y-1">
                    <span className="text-xs font-semibold text-neutral-200 block truncate">{p}</span>
                    <span className="text-[11px] text-neutral-400 font-mono">{count} captions</span>
                  </div>
                );
              })}
            </div>
        ...
        <div className="space-y-6">
          <div className="p-6 rounded-2xl bg-[#12151b] border border-[#202530] space-y-4">
            <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
              ...
              <div className="flex items-center gap-2">
                <button
                  ...
                  className="px-3.5 py-2 rounded-xl bg-[#1a202c] hover:bg-[#222a3a] border border-[#2d3748] text-neutral-200 text-xs font-medium flex items-center gap-2"
                >
                  <Plus className="w-3.5 h-3.5 text-amber-500" />
            ...
            {referenceWriters.map((writer) => (
              <div key={writer.id} className="p-5 rounded-xl bg-[#161a22] border border-[#262c37] space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h3 className="text-sm font-bold text-neutral-100">{writer.name}</h3>
                    <a
                    ...
                    <p className="text-xs text-neutral-400 mt-2">{writer.bio}</p>
                  </div>
                  <span className="px-2.5 py-1 rounded bg-[#1e2430] border border-[#2d3748] text-xs font-mono text-neutral-300">
                ...
                {writer.styleProfile && (
                  <div className="p-4 rounded-lg bg-[#11141a] border border-[#202530] space-y-3">
                    <span className="text-[11px] font-bold text-amber-400 uppercase tracking-wider block">
                      Extracted Generalized Structural Principles (Zero Phrasing Imitation)
                    </span>
                    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 text-xs">
                      <div>
                        <span className="text-neutral-500 block text-[10px] uppercase font-semibold">Tones</span>
                        <span className="text-neutral-200">{writer.styleProfile.tone?.join(", ")}</span>
                      </div>
                      <div>
                        <span className="text-neutral-500 block text-[10px] uppercase font-semibold">Hook Patterns</span>
                        <span className="text-neutral-200">{writer.styleProfile.hookPatterns?.join(", ")}</span>
                      </div>
                      <div>
                        <span className="text-neutral-500 block text-[10px] uppercase font-semibold">Paragraph Cadence</span>
                        <span className="text-neutral-200">{writer.styleProfile.paragraphStyle} (1-2 sentences)</span>
                      </div>
                      <div>
                        <span className="text-neutral-500 block text-[10px] uppercase font-semibold">Emotional Arc</span>
                        <span className="text-neutral-200">{writer.styleProfile.emotionalProgression}</span>
                      </div>
                      <div>
                        <span className="text-neutral-500 block text-[10px] uppercase font-semibold">Closing Style</span>
                        <span className="text-neutral-200">{writer.styleProfile.closingStyle}</span>
                      </div>
                      <div>
                        <span className="text-neutral-500 block text-[10px] uppercase font-semibold">Vocabulary</span>
                        <span className="text-neutral-200">{writer.styleProfile.vocabularyComplexity}</span>
                      </div>
                    </div>
                  </div>
                )}
            ...
            <div className="space-y-2">
              ...
              <div className="space-y-1.5">
                {(voiceProfile?.learnedRules || []).map((rule: string, idx: number) => (
                  <div key={idx} className="flex items-center gap-2 text-xs text-neutral-200 p-2 rounded-lg bg-[#161a22] border border-[#242b38]">
      ...
      {activeTab === "brand" && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          ...
          <div className="p-6 rounded-2xl bg-[#12151b] border border-[#202530] space-y-4">
            ...
            <h3 className="text-base font-bold text-neutral-100">Adetunwase Akanni Adenle</h3>
            ...
            <div className="space-y-2 pt-2 text-xs">
              <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider block">Verified Records</span>
          ...
          <div className="p-6 rounded-2xl bg-[#12151b] border border-[#202530] space-y-4">
            ...
            <h3 className="text-base font-bold text-neutral-100">Slum Art Foundation (Ijora Badia)</h3>
            <p className="text-xs text-neutral-300 leading-relaxed">
              Empowering children in slum communities through creative arts, animation, design, and environmental sustainability.
            </p>

            <div className="space-y-2 pt-2 text-xs">
              <span className="text-[10px] font-bold text-neutral-400 uppercase tracking-wider block">Key Initiatives</span>
              <ul className="space-y-1 text-neutral-300 list-disc list-inside">
      ...
      {activeTab === "settings" && (
        <div className="p-6 rounded-2xl bg-[#12151b] border border-[#202530] space-y-6 max-w-2xl">
          <h2 className="text-base font-bold text-neutral-100 flex items-center gap-2">
          ...
          <div className="space-y-4 text-xs">
            <div>
              <label className="text-neutral-300 font-semibold block mb-1">Daily Captions to Generate</label>
              <input
                type="number"
                value={settings?.dailyCaptionsCount || 7}
                onChange={(e) => setSettings({ ...settings, dailyCaptionsCount: parseInt(e.target.value, 10) })}
                className="bg-[#171b23] border border-[#262c37] rounded-lg px-3 py-2 text-neutral-200 w-full"
              />
              <span className="text-neutral-500 text-[11px] mt-1 block">Default: 7 captions per day.</span>
            </div>

            <div>
              <label className="text-neutral-300 font-semibold block mb-1">Minimum Backlog Target</label>
              <input
                type="number"
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                <label className="text-neutral-300 font-semibold block mb-1">Generation Time</label>
                <input
                  type="text"
                  value={settings?.generationTime || "06:00"}
                  onChange={(e) => setSettings({ ...settings, generationTime: e.target.value })}
                  className="bg-[#171b23] border border-[#262c37] rounded-lg px-3 py-2 text-neutral-200 w-full font-mono"
                />
              </div>
              <div>
                <label className="text-neutral-300 font-semibold block mb-1">Timezone</label>
                <input
                  type="text"
      ...
      {selectedItem && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#13161e] border border-[#28303f] rounded-2xl max-w-2xl w-full max-h-[90vh] flex flex-col overflow-hidden shadow-2xl">
            <div className="p-5 border-b border-[#202634] flex items-center justify-between">
              <div>
                <span className="text-[10px] font-bold text-amber-500 uppercase tracking-wider block">
                  {selectedItem.contentPillar} • {selectedItem.contentType}
                </span>
                <h3 className="text-base font-bold text-neutral-100">{selectedItem.title}</h3>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto space-y-4 flex-1 text-xs">
              {isEditing ? (
                <div className="space-y-3">
                  <div>
                    <label className="text-neutral-400 font-semibold block mb-1">Title</label>
                    <input
                      type="text"
                      value={editTitle}
                      onChange={(e) => setEditTitle(e.target.value)}
                      className="w-full bg-[#181d26] border border-[#2b3545] rounded-lg px-3 py-2 text-neutral-100 font-medium"
                    />
                  </div>
                  <div>
                    ...
                    <textarea
                      ...
                      className="w-full bg-[#181d26] border border-[#2b3545] rounded-lg p-3 text-neutral-100 font-normal leading-relaxed whitespace-pre-line"
                    />
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  <div className="p-4 rounded-xl bg-[#161a22] border border-[#242b38] leading-relaxed text-neutral-200 whitespace-pre-line text-xs">
                    {selectedItem.caption}
                  </div>

                  {selectedItem.hashtags && (
                    <div className="space-y-1">
                      <span className="text-[11px] font-semibold text-neutral-400">Hashtags:</span>
                  ...
                  {selectedItem.context && (
                    <div className="p-3 rounded-lg bg-[#101318] border border-[#1e2430] space-y-1 text-[11px]">
                      <span className="text-neutral-500 font-semibold uppercase block">Source Context</span>
                      <p className="text-neutral-300">{selectedItem.context.topic}: {selectedItem.context.description}</p>
                    </div>
                  )}
                </div>
              )}
            ...
            <div className="p-4 border-t border-[#202634] flex items-center justify-between gap-3 bg-[#11141b]">
              <div className="flex items-center gap-2">
                <button
                  onClick={() => handleCopy(selectedItem, "CAPTION")}
                  className="px-3 py-1.5 rounded-lg bg-[#1a202a] border border-[#2d3748] text-neutral-200 text-xs flex items-center gap-1.5"
                >
                  <Copy className="w-3 h-3 text-neutral-400" />
                  <span>Copy Caption</span>
                </button>
                ...
                {isEditing ? (
                  ...
                  <>
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-[#1e2430] border border-[#2b3545] text-neutral-200 flex items-center gap-1.5"
                    >
      ...
      {isContextModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleSubmitContext} className="bg-[#13161e] border border-[#28303f] rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            ...
            <textarea
              ...
              className="w-full bg-[#171b23] border border-[#262c37] rounded-xl p-3 text-xs text-neutral-200 placeholder-neutral-500 focus:outline-none focus:border-amber-500"
            />
      ...
      {isRefPostModalOpen && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <form onSubmit={handleAddRefPost} className="bg-[#13161e] border border-[#28303f] rounded-2xl max-w-lg w-full p-6 space-y-4 shadow-2xl">
            <div className="flex items-center justify-between">
              <h3 className="text-sm font-bold text-neutral-100 flex items-center gap-2">
                <BookOpen className="w-4 h-4 text-amber-500" />
            ...
            <input
              ...
              className="w-full bg-[#171b23] border border-[#262c37] rounded-xl px-3 py-2 text-xs text-neutral-200 placeholder-neutral-500"
            />
            ...
            <textarea
              ...
              className="w-full bg-[#171b23] border border-[#262c37] rounded-xl p-3 text-xs text-neutral-200 placeholder-neutral-500"
            />
      ...
      {schedulingItem && (
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#13161e] border border-[#28303f] rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-2xl">
            <h3 className="text-sm font-bold text-neutral-100 flex items-center gap-2">
            ...
            <div className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  className="w-full bg-[#171b23] border border-[#262c37] rounded-lg px-3 py-2 text-neutral-200"
                />
              </div>

              <div>
                <label className="text-neutral-400 block mb-1">Time (Africa/Lagos)</label>
                <input
                  type="time"
                  value={scheduledTime}
                  onChange={(e) => setScheduledTime(e.target.value)}
                  className="w-full bg-[#171b23] border border-[#262c37] rounded-lg px-3 py-2 text-neutral-200 font-mono"
                />
              </div>
            </div>

            <div className="flex justify-end gap-2 pt-2 text-xs">
              <button onClick={() => setSchedulingItem(null)} className="px-3 py-1.5 text-neutral-400">
                Cancel
              </button>
        ...
        <div className="fixed inset-0 z-50 bg-black/70 backdrop-blur-sm flex items-center justify-center p-4">
          <div className="bg-[#13161e] border border-[#28303f] rounded-2xl max-w-sm w-full p-6 space-y-4 shadow-2xl">
            ...
            <textarea
              ...
              className="w-full bg-[#171b23] border border-[#262c37] rounded-lg p-3 text-xs text-neutral-200 placeholder-neutral-500"
            />
            <div className="flex justify-end gap-2 text-xs">
              <button onClick={() => setRejectingItem(null)} className="px-3 py-1.5 text-neutral-400">
                Cancel
              </button>
