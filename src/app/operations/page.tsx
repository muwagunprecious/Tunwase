...
export default function OperationsPage() {
  ...
  return (
    <div className="flex-1 p-6 md:p-10 max-w-4xl mx-auto w-full space-y-8">
      {/* Header */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-[#1e222a] pb-6">
        <div>
          <h1 className="text-2xl font-bold text-neutral-100">Operations Hub</h1>
        ...
        <button
          ...
          className="px-3.5 py-2 rounded-xl bg-[#151921] hover:bg-[#1e2430] border border-[#262c37] text-neutral-300 text-xs font-medium flex items-center gap-2 transition-all self-start sm:self-center"
        >
      ...
      {whatToDoNow && (
        <div className="p-6 rounded-2xl bg-gradient-to-b from-[#161a22] to-[#12151b] border border-amber-500/30 space-y-4 shadow-lg">
          ...
          {whatToDoNow.nextSteps && whatToDoNow.nextSteps.length > 0 && (
            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2 pt-2 border-t border-[#202530] text-xs">
              {whatToDoNow.nextSteps.map((step: string, idx: number) => (
                <div key={idx} className="p-2.5 rounded-xl bg-[#181d26] text-neutral-200 text-xs">
      ...
      <form onSubmit={handleAddTask} className="relative flex items-center shadow-md">
        <input
          ...
          className="w-full bg-[#13161c] border border-[#252c38] focus:border-amber-500 rounded-2xl px-4 py-3.5 pr-28 text-sm text-neutral-100 placeholder:text-neutral-500 outline-none transition-all"
        />
        <button
          type="submit"
      ...
      <div className="space-y-4">
        <div className="flex items-center gap-2 border-b border-[#1e222a] pb-2 text-xs font-semibold">
          <button
            onClick={() => setActiveTab("tasks")}
            className={`px-3.5 py-1.5 rounded-lg transition-colors ${
              activeTab === "tasks" ? "bg-[#1f2533] text-white" : "text-neutral-400 hover:text-neutral-200"
            }`}
          >
          ...
          <button
            ...
            className={`px-3.5 py-1.5 rounded-lg transition-colors ${
              activeTab === "projects" ? "bg-[#1f2533] text-white" : "text-neutral-400 hover:text-neutral-200"
            }`}
          >
        ...
        {activeTab === "tasks" && (
          <div className="space-y-2">
            {tasks.length === 0 ? (
              <div className="p-8 text-center rounded-2xl bg-[#12151c] border border-[#202633] text-xs text-neutral-500">
                ...
                return (
                  <div
                    ...
                    className={`p-3.5 rounded-xl border transition-all flex items-center justify-between gap-3 text-xs ${
                      isCompleted
                        ? "bg-[#111317]/50 border-[#1c2027] text-neutral-500"
                        : "bg-[#13161c] border-[#222834] text-neutral-200 hover:border-[#2f3747]"
                    }`}
                  >
                    ...
                    <div className="flex items-center gap-3 shrink-0">
                      {task.entity && (
                        <span className="text-[9px] px-2 py-0.5 rounded bg-[#181d26] text-neutral-400">
        ...
        {activeTab === "projects" && (
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            {projects.map((proj: any) => (
              <div
                key={proj.id}
                className="p-5 rounded-2xl bg-[#13161c] border border-[#222834] space-y-3"
              >
                <div className="flex items-start justify-between gap-2">
                  <div>
                    <span className="text-[9px] px-2 py-0.5 rounded bg-[#181d26] text-neutral-400 font-bold uppercase">
                ...
                <p className="text-xs text-neutral-400 line-clamp-2">{proj.objective}</p>
                  ...
                  <div className="pt-2 border-t border-[#1f2533] space-y-1 text-xs">
                    ...
                    {proj.milestones.slice(0, 3).map((m: any, idx: number) => (
                      <div key={idx} className="flex items-center justify-between text-[11px]">
                        <span className={m.completed ? "text-neutral-500 line-through" : "text-neutral-300"}>
                          {m.title}
                        </span>
                        <span className="text-neutral-500 text-[10px]">{m.targetDate}</span>
                      </div>
                    ))}
                  </div>
                )}
